...
from models.model_MLPOmics import MLPOmics,MLPOmicswithP
...
from models.model_SurvPath_mine import SurvPath_with_Plugin,SurvPath_with_Plugin1
...
def _init_optim(args, model):
    ...
    if args.opt == "adam":
        ...
        optimizer = Lambda(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, weight_decay=args.reg)
    else:
        raise NotImplementedError
...
def _unpack_data(modality, device, data):
    ...
    if modality in ["mlp_per_path", "omics", "snn"]:
        ...
        y_disc, event_time, censor, clinical_data_list, mask,slide_id = data[7], data[8], data[9], data[10], data[11],data[12]
        mask = mask.to(device)

    elif modality in ["survpath","survpath_mine"]:
        ...
        y_disc, event_time, censor, clinical_data_list,slide_id = data[2], data[3], data[4], data[5],data[7]
        return data_WSI, mask, y_disc, event_time, censor, data_omics, clinical_data_list, mask,slide_id
...
def _process_data_and_forward(model, modality, device, data):
    ...
    """
    data_WSI, mask, y_disc, event_time, censor, data_omics, clinical_data_list, mask,slide_id = _unpack_data(modality, device, data)
    if modality in ["coattn", "coattn_motcat"]:  
        ...
        input_args['slide_id'] = slide_id
        input_args['clinical_data'] = clinical_data_list
...
def l1_reg_modules(model):
    l1_reg = 0

    l1_reg += l1_reg_all(model.cpks_plugin.align)
